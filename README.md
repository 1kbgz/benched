...
<benched-report src="data/report.json" view="trend" metric="median" x-axis="version"></benched-report>
```
...
the x-axis by default; set `x-axis="time"` to use run time instead. Theme it through inherited
text styles and the `--benched-accent-color` and `--benched-grid-color` CSS properties.
Color mode follows the browser preference until the report's top-right theme button
stores a local light/dark override. Set `data-theme="light"` or `data-theme="dark"` on
the component for an explicit initial mode.
